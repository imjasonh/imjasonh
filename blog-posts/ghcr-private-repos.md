...
![Text reads, "Dear Registry, I'm about to send you layer sha256:abcdef to repo production, but if you already have that layer in repo staging, just copy it from there. Sincerely, Eustace P. Customer I V, E S Q., D D S."](ghcr-private-repos-1.png)
...
The problem arises when you store multiple tenants’ worth of data in one bucket, and serve it from a single user-facing frontend service. This seems to be how GHCR is implemented, as a single ABS bucket, named ghcr1production, with every GHCR repo’s blobs stored together.
...
For reporting the bug, GitHub awarded a $617 reward. The reward was donated to [Women Have Options](https://abortionfunds.org/fund/abortion-fund-of-ohio/), an abortion access fund in Ohio, which GitHub matched. Chainguard contributed another $1,234, bringing the total contribution to $2,468.
